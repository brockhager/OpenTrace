...
if (form) {
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      statusEl.textContent = 'Search failed ΓÇö try again.';
    }
  });
}
  ...
  useLocationBtn.addEventListener('click', async () => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        useLocationBtn.textContent = 'Γ£ô Using my location';
...
if (loginForm) {
  loginForm.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      loginStatus.textContent = 'Sign-in failed ΓÇö check credentials';
    }
  });
}
...
function renderRow(p){
  ...
  return `
    <div class="card">
      ...
      <p>${escapeHtml(location)} ΓÇö ${escapeHtml(author)}</p>
...
function renderCard(p) {
  ...
  return `
    <article class="card">
      <span class="card-id">${escapeHtml(p.pfif_id || '')}</span>
      <h3>${escapeHtml(name)} <span class="meta">(${p.age_at_disappearance || 'ΓÇö'}, ${p.sex || 'ΓÇö'})</span></h3>
...
if (personsForm) {
  personsForm.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      personsStatus.textContent = 'Search failed ΓÇö try again.';
    }
  });
...
if (locationsForm) {
  locationsForm.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      locationsStatus.textContent = 'Search failed ΓÇö try again.';
    }
  });
}
    ...
    eventsResults.innerHTML = events.map(event => {
      ...
      return `
        <article class="card">
          ...
          <p><strong>Timestamp:</strong> ${escapeHtml(event.event_timestamp || 'ΓÇö')}</p>
          <p class="meta">Person: ${escapeHtml(event.person_id || 'ΓÇö')} ┬╖ Location: ${escapeHtml(event.location_id || 'ΓÇö')}</p>
  ...
  try {
    ...
    sourcesResults.innerHTML = sources.map(source => `
      <article class="card">
        ...
        <p class="meta">Reliability: ${source.reliability_score ?? 'ΓÇö'}</p>
...
async function loadPersonDetail() {
  ...
  try {
    ...
    personDetailEl.innerHTML = `
      ...
      <p><strong>Status:</strong> ${escapeHtml(person.status || 'ΓÇö')}</p>
      <p><strong>Age at disappearance:</strong> ${person.age_at_disappearance ?? 'ΓÇö'}</p>
      <p><strong>Sex:</strong> ${escapeHtml(person.sex || 'ΓÇö')}</p>
      <p><strong>Last seen:</strong> ${escapeHtml(person.date_last_seen || 'ΓÇö')}</p>
      ...
      <section class="detail-section">
        ...
        ${timeline.length ? timeline.map(evt => `
          <div class="card">
            ...
            <p class="meta">${escapeHtml(evt.event_timestamp || 'ΓÇö')}</p>
...
async function loadLocationDetail() {
  ...
  try {
    ...
    locationDetailEl.innerHTML = `
      ...
      <p><strong>Canonical Name:</strong> ${escapeHtml(location.canonical_name || 'ΓÇö')}</p>
      <p><strong>Type:</strong> ${escapeHtml(location.location_type || 'ΓÇö')}</p>
      <p><strong>Coordinates:</strong> ${location.latitude ?? 'ΓÇö'}, ${location.longitude ?? 'ΓÇö'}</p>
      <p><strong>Country:</strong> ${escapeHtml(location.country_name || 'ΓÇö')} (${escapeHtml(location.country_code || 'ΓÇö')})</p>
      <p><strong>Admin1 (State/Province):</strong> ${escapeHtml(location.admin1_name || 'ΓÇö')}</p>
      <p><strong>Locality (City):</strong> ${escapeHtml(location.locality || 'ΓÇö')}</p>
      <p class="meta">Confidence: ${location.confidence_score ?? 'ΓÇö'}</p>
    `;
...
async function loadEventDetail() {
  ...
  try {
    ...
    eventDetailEl.innerHTML = `
      ...
      <p><strong>Type:</strong> ${escapeHtml(event.event_type || 'ΓÇö')}</p>
      <p><strong>Timestamp:</strong> ${escapeHtml(event.event_timestamp || 'ΓÇö')}</p>
      <p><strong>Person:</strong> ${escapeHtml(event.person_id || 'ΓÇö')}</p>
      <p><strong>Location:</strong> ${escapeHtml(event.location_id || 'ΓÇö')}</p>
      <p><strong>Confidence:</strong> ${escapeHtml(event.confidence || 'ΓÇö')}</p>
      <p><strong>Visibility:</strong> ${escapeHtml(event.visibility || 'ΓÇö')}</p>
...
if (urlScrapeForm) {
  urlScrapeForm.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (res.found) {
        urlStatus.textContent = `Γ£à ${res.message}${res.created ? ' (New record created)' : ' (Already exists)'}`;
        if (res.person) {
          urlStatus.textContent += ` - ${res.person.given_name || ''} ${res.person.family_name || ''}`;
        }
      } else {
        urlStatus.textContent = `ΓÜá∩╕Å ${res.message}`;
      }
      
      urlScrapeForm.reset();
    } catch (err) {
      urlStatus.textContent = `Γ¥î Scraping failed: ${err.message}`;
    }
  });
}
...
if (genericUrlScrapeForm) {
  genericUrlScrapeForm.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (res.found) {
        genericUrlStatus.textContent = `Γ£à ${res.message}${res.created ? ' (New record created)' : ' (Already exists)'}`;
        if (res.person) {
          genericUrlStatus.textContent += ` - ${res.person.given_name || ''} ${res.person.family_name || ''}`;
        }
      } else {
        genericUrlStatus.textContent = `ΓÜá∩╕Å ${res.message}`;
      }
      
      genericUrlScrapeForm.reset();
    } catch (err) {
      genericUrlStatus.textContent = `Γ¥î Scraping failed: ${err.message}`;
    }
  });
}
...
if (pdfScrapeForm) {
  pdfScrapeForm.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (!file) {
        pdfStatus.textContent = 'Γ¥î Please select a PDF file';
        return;
      }
      ...
      if (res.ok && data.found) {
        pdfStatus.textContent = `Γ£à ${data.message}${data.created ? ' (New record created)' : ' (Already exists)'}`;
        if (data.person) {
          pdfStatus.textContent += ` - ${data.person.given_name || ''} ${data.person.family_name || ''}`;
        }
      } else {
        pdfStatus.textContent = `ΓÜá∩╕Å ${data.message || 'Failed to process PDF'}`;
      }
      
      pdfScrapeForm.reset();
    } catch (err) {
      pdfStatus.textContent = `Γ¥î Processing failed: ${err.message}`;
    }
  });
}
